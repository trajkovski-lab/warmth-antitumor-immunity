//This macro will segment fibroblast based on user defined threshold methods
//it will also segment nuclei using cellpose

////////////////////////////////////////////
//User defined parameters
seriesName = "all"; //if "all", it will work on all the elements of the series. Specify a string if you want to analyse only images containing a particular word in the title
C3method="Triangle"; 
C4method="Moments"; 
C2method="Moments";
C3minsize="0"; 
	...
	Array.show(seriesToOpen);
		...
		for(s = 0; s < seriesToOpen.length; s++) {
			...
			selectWindow(imageName);
					...
					if(!File.exists(dir +"C2-" +  imageName + ".zip") ){
	...
	Array.show(seriesToOpen);
		...
		for(s = 0; s < seriesToOpen.length; s++) {
			...
			if(!File.exists( dir + imageName + "_Eritros.zip") ){
				...
				for (ch=2; ch<5; ch++) {
					//if we have the eritrocite areas segmented saved as "_Eritros.zip", it will open this file and exclude them from the threshold segmentation
