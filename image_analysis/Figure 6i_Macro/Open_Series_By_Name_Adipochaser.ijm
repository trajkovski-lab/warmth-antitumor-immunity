...
seriesName = "all"; //"all" will analise all images. Introduce string here if only images with a specific string in title should be analysed
C3method="Triangle"; 
C4method="Moments"; 
C2method="Moments";
C3minsize="30"; //segmentations below these area size will be discarded, useful to remove noise
C4minsize= "10";
C2minsize= "50";
C3dilates= 4; //dilates of segmentations performed, useful to remove holes
C4dilates= 2;
C2dilates= 2;
minthreshold = 22;
maxthreshold = 255;
	...
	Array.show(seriesToOpen);
		...
		for(s = 0; s < seriesToOpen.length; s++) {
			...
			selectWindow(imageName);
				...
				//adipochasing(imageName); //adipochasing is the function with all the instructions, I define it at the end
				run("Close All");
