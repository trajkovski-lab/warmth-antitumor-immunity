...
imagetype="fluo"; //either fluo or histo. For brightfield images, put "fluo" too.
BF=true; //put true here if brightfield
...
enhancecontrastfluo=false; // put true if you want the contrat of the fluo image to be increased. It will saturate a percentage of the pixels. 
percsaturated=0.35; // percentage of pixels saturated if enhancecontrastfluo is set to true
blurimagefluo=false; //recommended false. True if image too noisy, or too many small segmentations found. It sacrifices small LD segmentations, but sometimes it compensates
...
diambig=20; // 25 for big LDs 100 for cav1 adipocyte segmentation
//optimaze for each experiment
flowbig=-2.0;
probbig=2.0;
modelbig="cyto2"; 

//settings for find maxima to eliminate big LDs that merge too much
testmaxima=true;
topmaximapoints=2; // big selections with more maxima points than these, will be eliminated
maximaprominence=20; //fluo was 20; //maxima detection sensitivity, smaller prominence will give more maxima points, increase if too many big LDs eliminated
dosigmamaxima=true; //should image be blurred before running maxima?
...
maxareabr=999999999999999999999; //100;
minmeanbr=0; //150;
//parameters to exclude big
//minimum intensity allowed
minmeanbig=0; //175 histo

//maximum area and min cir: all segmentations above this area or below this circ will be removed
maxarea=50000000000000000000000000000000000000000000000000000; 
mincirc=0.6;
//parameters for abhorrent shapes: segmentations that are bigger and less circular than these will be removed
maxareac=99999999999999999999999999999999999999; 
mincircc=0.6;
minarea=1;


//parameters for image series and image cropping
resolution=3; //maginification you want to analyse. Higly recommend 3 (3rd best); If more resolution (1 or 2), the image will need to be cropped. Less resolution (>3) will greatly compromise small LD segmentation. If only big LD, not problem with using >3
					...
					for ( j=0; j<ncropsy; j++) {
						...
						if (dosmall== true) {
							if(!File.exists(dir + image +"_" + i + "_"+ j +  "_ch" +channelld+"_small0.zip")) {
								...
								//run("Cellpose Advanced", "diameter=5 cellproba_threshold=-6.0 flow_threshold=5 anisotropy=1.0 diam_threshold=100.0 model=cyto2 nuclei_channel=2 cyto_channel=1 dimensionmode=2D stitch_threshold=-1.0 omni=false cluster=false additional_flags=");
							...
							if (nbrois >0 ){
								if(!File.exists(dir + image +"_" + i + "_"+ j +   "_ch" +channelld+"_big1.zip")) {
									...
									for (k=0; k<nResults; k++) {
										...
										if( (area > maxareabr && mean < minmeanbr) || (area< minarea) ||(area> maxarea) || (area > maxareac && circ < mincircc) || (circ < mincirc)  || (mean < minmeanbig ) ){ 
